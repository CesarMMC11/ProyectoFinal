...
// Inscribirse en una clase (para usuarios con rol 'user' o 'admin')
const inscribirEnClase = async (req, res) => {
    try {
        ...
        // Respuesta exitosa
        res.status(201).json({ message: 'Inscripción a la clase creada exitosamente', inscripcion: nuevaInscripcion });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Error al inscribirse en la clase', error: error.message });
    }
};
...
const obtenerInscripciones = async (req, res) => {
    try {
        ...
        const inscripciones = await Clase.findAll({
            include: [{ model: User, as: 'user', attributes: ['name', 'fecha', 'telefono', 'hora'] }], // Incluye información del usuario
        });
